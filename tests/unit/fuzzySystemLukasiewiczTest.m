classdef fuzzySystemLukasiewiczTest < matlab.unittest.TestCase
    % Published and exhaustive-oracle tests for min-Lukasiewicz systems.
    ...
    methods (Test)
        function testPublishedEquationExample(testCase)
            ...
            ex = fixture.Lukasiewicz;
            system = fuzzySystem('lukasiewicz', ex.a, ex.b, [], true);
        ...
        function testOneBoundaryAndInconsistentSystem(testCase)
            oneBoundary = fuzzySystem('lukasiewicz', ...
            ...
            inconsistent = fuzzySystem('lukasiewicz', ...
        ...
        function testEquationAndInequalitiesAgainstIndependentOracle(testCase)
            for repetition = 1:testCase.DevelopmentRepetitions
                rng(7300 + repetition, 'twister');
                a = fuzzyMatrix(randi([0 4], 2, 3) / 4);
                witness = fuzzyMatrix(randi([0 4], 3, 1) / 4);
                b = lukasiewicz(a, witness);

                for inequalities = [-1 0 1]
                    expected = referenceLukasiewiczSolutions(a, b, inequalities);
                    actual = fuzzySystem( ...
                        'lukasiewicz', a, b, [], true, inequalities);

                    context = sprintf(['min-Lukasiewicz oracle mismatch\n' ...
